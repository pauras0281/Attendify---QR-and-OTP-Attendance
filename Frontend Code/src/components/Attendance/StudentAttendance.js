...
const StudentAttendance = () => {
  ...
  useEffect(() => {
    const fetchStudentAttendance = async () => {
      try {
        // Fetch lectures
        const lectureResponse = await axios.get('http://localhost:8080/api/lectures');
        const lectureData = await lectureResponse.data.lectures;

        // Fetch student's attendance
        const studentResponse = await axios.get('http://localhost:8080/api/user/students/allstudents');
        const studentData = await studentResponse.data;

        // Filter student's attendance for the logged-in student
        const loggedInStudent = studentData.find(student => student._id === sessionStorage.getItem('userId'));
        ...
        if (loggedInStudent) {
          const studentAttendanceData = lectureData.map(lecture => {
            const attendance = {
              ...
              attendanceStatus: lecture.attendedStudents.includes(loggedInStudent._id) ? 'Present' : 'Absent'
            };
            return attendance;
          });
          ...
          <tbody>
            {studentAttendance.map((attendance, index) => (
              <tr key={index} className="attendance-row">
                ...
                <td className={attendance.attendanceStatus === 'Present' ? 'present' : 'absent'}>
