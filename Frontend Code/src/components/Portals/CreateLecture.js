...
const CreateLecture = ({onClick}) => {
  ...
  const createLecture = async () => {
    try {
      ...
      // Handle further actions after successful creation, if needed
  ...
  const fetchSubjects = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8080/api/sub/`); // Replace 'lectureId' with the actual ID of the lecture
